...
def test_normalize_entity_strips_case_punctuation_legal_honorific():
    assert normalize_entity("Government of Norway") == "government of norway"
    assert normalize_entity("GOVERNMENT, of  Norway.") == "government of norway"
...
def test_normalize_resolver_merges_variants_and_remaps_edges():
    g = _two_donor_graph("Government of Norway", "GOVERNMENT OF NORWAY,")
    merged = resolve_graph(g, NormalizeResolver(key="name"))
    donors = list(merged.nodes_of_type("donor"))
    assert len(donors) == 1
    canon = donors[0].id
    assert canon == "donor:government-of-norway"
...
def test_distinct_entities_are_not_merged():
    g = _two_donor_graph("Government of Norway", "European Commission")
    merged = resolve_graph(g, NormalizeResolver(key="name"))
...
def test_alias_table_merges_known_synonyms():
    g = _two_donor_graph("Government of Norway", "Norwegian MFA")
    r = NormalizeResolver(key="name", aliases={"norwegian mfa": "government of norway"})
    merged = resolve_graph(g, r)
    assert len(list(merged.nodes_of_type("donor"))) == 1


def test_registry_resolver_uses_canonical_ids():
    g = _two_donor_graph("Government of Norway", "Norway (Govt)")
    registry = {"government of norway": "org:NO-301", "norway govt": "org:NO-301"}
    merged = resolve_graph(g, RegistryResolver(registry, key="name"))
    donors = list(merged.nodes_of_type("donor"))
    assert len(donors) == 1 and donors[0].id == "org:NO-301"


def test_llm_resolver_with_fake_judge():
    g = _two_donor_graph("Government of Norway", "the Norwegian state")
    # cheap normalize can't match these; the (fake) LLM judge says they're the same
    judge = lambda a, b: "norw" in a.lower() and "norw" in b.lower()
...
def test_facade_resolve_pass_merges_messy_extractions():
    ...
    g = extract(
        "Donor: Government of Norway\nDonor: Government of Norway Inc",
        spec,
